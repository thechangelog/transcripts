...
**Adam Stacoviak:** \[32:16\] Right. So it goes stale over time. I might be a few revs behind.
...
So that's one tack that we're taking on this problem, which is we just want people to be able to run on a machine with an NVIDIA GPU or an AMD GPU or whatever, just run Uv add Torch vLLM flash-attention and not have to think about building from source, not have to think about where are these coming from, not having to think about making sure that the CUDA versions are all lined up. That's part of the complexity that we're trying to solve.
...
**Adam Stacoviak:** Yeah, out of respect, makes sense.
