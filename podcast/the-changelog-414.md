...
**Matthew Hodgson:** Yeah. I mean, if nothing else, we'll be adding GitLab authentication integration into all of Element's, as well as going and bringing "every repository can be a chat room" mentality from Gitter into the wider Matrix ecosystem. As part of getting Element at parity with Gitter we need to have that sort of feature, so I think there could definitely be the DevOps/ChatOps layer, even if it's driven mainly from the Matrix side, rather than the GitLab side. And perhaps we'll get to the point where it will be an obvious -- I'm not gonna use the s-word... An obvious integration between the two of some flavor.
...
**Matthew Hodgson:** The irony is if I wanted to make it sound cool, I've gotta kind of go into "In a world..." Doing this voice in a really bad American cinema accent...
