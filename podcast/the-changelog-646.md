...
\[12:03\] And by the way, htmx can make it even better, and make it more effective. And then the advantages of that old approach, which were simplicity - there's a simplicity to it, and so forth - it can be available to you, even if you're building some subset of modern applications. It's not appropriate for everything, of course, and so I try to talk about the tradeoffs that are inherent in hypermedia-based applications... But it can be used for a lot of stuff. And even if you don't use it, I think it's interesting from a technical perspective just to understand the ideas and what the tradeoffs are.
