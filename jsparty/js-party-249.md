...
**Michael Gearon:** And then what's quite interesting with the grid area is you can then combine it with like a media query, just to say as the layout changes, as your browser width or wherever that media query changes, you can then update the grid template area to say, "I want to rearrange that layout to make more sense, so you could have a different experience for mobile as you would for desktop."
