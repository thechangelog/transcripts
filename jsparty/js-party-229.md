...
For example, if you wanted to get like an A, for an alert, in order to do an alert - aren't alerts deprecated now? Anyways... If you wanted to write an alert, an array is truthy, so in order to get the word false, you could do bang and then an empty array. You can get true by doing bang bang and then that empty array, too.
...
**Ali Spittel:** Yeah, so this is more of a programming oddity; and it's not even really an oddity, it's how it's supposed to be... But if you've ever done 0.1 + 0.2 in JavaScript, you'll notice that you get like three point and then a bunch of zeroes and then a four. You've probably done this before; you've done like float math and seen this really random number show up. I've have it happen if a handle, like a shopping cart or whatever, with floats instead of strings, or instead of doing the integer math, or whatever.
...
I can tell you, we're right now - it's hard to filter through bootcamp grads. We've got hundreds of bootcamp grads applying, and it takes a lot of time and energy. And we care a lot, so we're figuring out "How do we work through this?", but it is really hard, and a lot of companies will just say "No. You know what - if you don't have any track record, I'm not even gonna bother talking with you." So looking for ways to manufacture that track record, and one way you can do that is getting involved with open source. And if you can show a history of contribution to a project, that is a track record. The people who are on that project are going to know of you, they become a protonetwork, a set of people. So I hesitate to point everyone there, because it does take a time commitment, and not everyone has the ability to put that time in. But if you do, that can be a great way to hack around that lack of track record.
...
We'll catch you all next week. If you're listening and you wanna join us live, you can always join us live on Thursdays at 10 AM Pacific, 1 o'clock Eastern, and who knows what it is in Omaha... But we'll play with y'all. Alright, take it easy, y'all.
